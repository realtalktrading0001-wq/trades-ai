...
The whole UI is driven by `user.status: 'unregistered' | 'verifying' | 'verified'`.

**The verification flow is the core domain logic.** It is the single seam to make real:
1. `POST /api/registration/id` stores the PocketOption ID, sets `status='verifying'`, stamps `verify_started_at`.
2. `GET /api/registration/status` calls `maybeVerify()` (in `state.ts`), which promotes the
   user to `verified` once `VERIFY_DELAY_SECONDS` have elapsed — a **simulated** approval.
   To wire real verification (e.g. a PocketOption affiliate postback), replace `maybeVerify()`
   only; everything else keys off `status`.
3. The Signals screen polls `/api/me` every 2s while `status === 'verifying'`.
