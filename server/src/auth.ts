...
function ensureUser(tgId: string, name: string, startParam?: string): UserRow {
  ...
  if (!row) {
    ...
    db.prepare(
      `INSERT INTO users (tg_id, name, ref_code, referred_by, attrib_fbc, attrib_fbp, attrib_source, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(tgId, name, code, referredBy, attribFbc, attribFbp, attribSource, Date.now());
